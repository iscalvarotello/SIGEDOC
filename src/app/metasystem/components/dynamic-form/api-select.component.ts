  ...
  template: `
    <div class="relative">
      @if (field().apiConfig?.multiple) {
        <div class="max-h-48 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-2 bg-gray-50 dark:bg-gray-900"
          ...
          @if (isLoading()) {
            <div class="p-4 text-center text-gray-500 text-sm flex items-center justify-center gap-2">
               <svg class="animate-spin h-4 w-4 text-theme-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              @for (opt of options(); track opt.value) {
                <label class="flex items-center gap-3 cursor-pointer p-1.5 hover:bg-white dark:hover:bg-gray-800 rounded transition-colors group">
                  <input type="checkbox" 
                         ...
                         class="w-4 h-4 text-theme-primary bg-white border-gray-300 rounded focus:ring-theme-primary focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        ...
        <select 
          [formControl]="control()"
          class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-theme-primary/50 focus:border-theme-primary transition-all disabled:opacity-50 disabled:cursor-not-allowed appearance-none"
        ...
        @if (isLoading()) {
          <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none">
            <svg class="animate-spin h-5 w-5 text-theme-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
          </div>
