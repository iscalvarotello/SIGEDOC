...
export abstract class BaseFormController<T> implements OnInit {
  ...
  public globalErrors = signal<string[]>([]);
  ...
  protected async loadData(id: string) {
    ...
    try {
      ...
      const msg = error?.error?.message || 'El registro ya no existe o la información está desactualizada.';
  ...
  public async onSave() {
    ...
    // Omitir llaves auxiliares que no son parte del DTO
    if (this.controllerConfig.keysToOmitOnSubmit) {
      this.controllerConfig.keysToOmitOnSubmit.forEach(key => {
        delete formValue[key];
      });
    }

    try {
      this.globalErrors.set([]); // Limpiar errores antes de guardar
      ...
      if (unmappedErrors.length > 0) {
        this.globalErrors.set(unmappedErrors);
      }
