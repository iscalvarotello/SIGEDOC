...
import { ViewerSelectorComponent } from '@system-shared/media/viewer-selector/viewer-selector.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ActionButtonComponent, ViewerSelectorComponent, IconComponent],
...
export class DocumentViewerPageComponent implements OnInit, OnDestroy {
  ...
  async onRegeneratePdf() {
    ...
    try {
      await this._documentService.regeneratePdf(docId);
      this._documentService.clearCachedPdf(docId);
      
      await this.loadDocumentData(docId);
    } catch (err: any) {
  ...
  closeViewer() {
    try {
      // Intentar cerrar la ventana si se abrió en una nueva pestaña
      window.close();
    } catch (e) {}

    // Como fallback, regresar en el historial
    this._location.back();
  }
