...
export class EmployeeFormComponent implements OnInit, OnChanges {
  ...
  async loadFormCatalogs() {
    try {
      ...
      const mapped = (persons || []).map(p => ({
        ...
        rfc: p.rfc,
        job_name: p.employee?.id ? 'Empleado (Sin Adscripción)' : 'Persona registrada',
        area_name: '',
        existing_employee_id: p.employee?.id || null
      }));
  ...
  onPersonSelect(person: any) {
    ...
    const existingEmpControl = this.form.get('existing_employee_id');
    if (existingEmpControl) {
      existingEmpControl.setValue(person.existing_employee_id);
    }
  }
  ...
  onPersonClear() {
    ...
    const existingEmpControl = this.form.get('existing_employee_id');
    if (existingEmpControl) {
      existingEmpControl.setValue(null);
    }
  }
  ...
  async onSubmit() {
    ...
    try {
      if (action === 'nuevo') {
        let employeeId = rawVal.existing_employee_id;
        ...
        // PASO 2: Adscribir al empleado creado (o existente) al área correspondiente
        const adscriptionPayload = {
          employee_id: employeeId, // UUID del empleado
          area_id: area.id,
