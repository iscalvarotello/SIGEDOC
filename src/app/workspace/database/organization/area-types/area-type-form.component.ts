...
import { AREA_TYPE_FORM_CONFIG } from './area-type-form.config';
import { IconComponent } from '@system-shared/common/icon/icon.component';

@Component({
  selector: 'app-area-type-form',
  standalone: true,
      ...
      <div class="mb-6 flex justify-between items-center border-b border-gray-100 dark:border-gray-700 pb-4">
        <div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ isEditMode() ? 'Editar Tipo de Área' : 'Nuevo Tipo de Área' }}
          </h2>
          <p class="text-sm text-gray-500 mt-1">
            Complete el formulario para registrar o actualizar un tipo de área en el sistema.
          </p>
        </div>
      </div>
...
export class AreaTypeFormComponent extends BaseFormController<AreaTypeDTO> {
  protected apiService = inject(AreaTypeService);
  protected controllerConfig = AREA_TYPE_FORM_CONFIG;
}
