...
import { INTERNAL_TEMPLATE_FORM_CONFIG } from './internal-template-form.config';

@Component({
  selector: 'app-internal-template-form',
  standalone: true,
      ...
      <div class="mb-6 flex justify-between items-center border-b border-gray-100 dark:border-gray-700 pb-4">
        <div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ isEditMode() ? 'Editar Plantilla Interna' : 'Nueva Plantilla Interna' }}
          </h2>
          <p class="text-sm text-gray-500 mt-1">
            Complete el formulario para configurar la plantilla de cuerpo de documento.
          </p>
        </div>
      </div>
...
export class InternalTemplateFormComponent extends BaseFormController<InternalTemplateDTO> {
  protected apiService = inject(InternalTemplateService);
  protected controllerConfig = INTERNAL_TEMPLATE_FORM_CONFIG;
}
