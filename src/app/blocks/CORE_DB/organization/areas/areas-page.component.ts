...
import { FilterAreaComponent       } from '@shared/components/filters/filter-area.component';
import { ActionBarAction           } from '@shared/components/common/action-bar/action-bar.component';
import { ActionBarComponent        } from '@shared/components/common/action-bar/action-bar.component';
...
export class AreasPageComponent extends BasePageController<AreaDTO> {
  ...
  override filteredItems = computed(() => {
    ...
    if (this.pageConfig.sortConfig) {
      const key = this.pageConfig.sortConfig.key as keyof AreaDTO;
      const dir = this.pageConfig.sortConfig.direction === 'asc' ? 1 : -1;
      items = [...items].sort((a, b) => {
        // Orden principal por index_sort (nulls al final)
        const valA = a[key] !== null && a[key] !== undefined ? Number(a[key]) : Infinity;
        const valB = b[key] !== null && b[key] !== undefined ? Number(b[key]) : Infinity;
    ...
    return items.filter(item => {
      return this.pageConfig.searchFields?.some(field => {
        const val = item[field as keyof AreaDTO];
