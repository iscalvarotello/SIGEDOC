...
import { PARTIDA_FORM_CONFIG } from './partida-form.config';
      ...
      <div class="mb-6 flex justify-between items-center border-b border-gray-100 dark:border-gray-700 pb-4">
        <div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ isEditMode() ? 'Editar Partida Presupuestal' : 'Nueva Partida Presupuestal' }}
          </h2>
          <p class="text-sm text-gray-500 mt-1">
            Complete el formulario para registrar o actualizar una partida presupuestal en el sistema.
          </p>
        </div>
      </div>
...
export class PartidaFormComponent extends BaseFormController<PartidaDTO> {
  protected apiService = inject(PartidaService);
  protected controllerConfig = PARTIDA_FORM_CONFIG;
}
