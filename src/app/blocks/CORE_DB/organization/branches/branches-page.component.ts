...
import { BRANCH_PAGE_CONFIG } from './branch-page.config';
...
@Component({
  ...
  template: `
    <app-master-wrapper 
      ...
      searchPlaceholder="Buscar por nombre, dirección, ciudad..."
...
export class BranchesPageComponent extends BasePageController<BranchDTO> {
  protected apiService = inject(BranchService);
  public pageConfig = BRANCH_PAGE_CONFIG;
  ...
  override filteredItems = computed(() => {
    ...
    if (this.pageConfig.sortConfig) {
      const key = this.pageConfig.sortConfig.key as keyof BranchDTO;
    ...
    return items.filter(item => {
      return this.pageConfig.searchFields?.some(field => {
        const val = item[field as keyof BranchDTO];
