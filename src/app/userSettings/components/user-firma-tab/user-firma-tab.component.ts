...
export class UserFirmaTabComponent implements OnInit {
  ...
  async uploadFirmaFile() {
    ...
    try {
      console.log(`[FirmaTab] Invocando employeeService.uploadFirma para empleado ${employeeId}...`);
      const response = await this.employeeService.uploadFirma(employeeId, file);
      console.log('[FirmaTab] Peticion HTTP completada exitosamente. Respuesta del servidor:', response);
      alert('Firma electrónica (.p12) guardada correctamente.');
      this.selectedFirmaFile.set(null);
      ...
      if (currentUser) {
        ...
        localStorage.setItem('sigedoc_user_data', JSON.stringify(currentUser));
      }
    } catch (err: any) {
