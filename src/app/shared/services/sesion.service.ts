...
import { PermissionDTO } from '@core_db/security/permissions/permission.dto';
...
export class SesionService {
  ...
  // Señales de Estado
  public token = signal<string | null>(localStorage.getItem('sigedoc_token'));
  public currentUserData = signal<any | null>(JSON.parse(localStorage.getItem('sigedoc_user_data') || 'null'));
  
  // Adscripción activa
  public activeAdscription = signal<any | null>(JSON.parse(localStorage.getItem('sigedoc_active_adscription') || 'null'));
  ...
  public photoError = signal<boolean>(false);

  // Computed helper para obtener la URL de la foto de perfil real
  public profilePhotoUrl = computed(() => {
    const user = this.currentUserData();
    if (!user || !user.id_usuario) return '';
    const baseUrl = environment.URL_PATH.replace(/\/$/, '');
    return `${baseUrl}/users/usuarios/profilephoto/${user.id_usuario}?t=${this.photoTimestamp()}`;
  });
  ...
  async setSession(loginResponse: { user: any; token: string }) {
    ...
    localStorage.setItem('sigedoc_token', loginResponse.token);
    localStorage.setItem('sigedoc_user_data', JSON.stringify(loginResponse.user));
    ...
    if (adsList.length > 0) {
      ...
      localStorage.removeItem('sigedoc_active_adscription');
    }
  ...
  setActiveAdscription(adscription: any) {
    ...
    localStorage.setItem('sigedoc_active_adscription', JSON.stringify(adscription));
  }
  ...
  clearSession() {
    ...
    localStorage.removeItem('sigedoc_token');
    localStorage.removeItem('sigedoc_user_data');
    localStorage.removeItem('sigedoc_active_adscription');
  }
  ...
  updatePersonalData(newPersonalData: any) {
    ...
    if (user) {
      ...
      localStorage.setItem('sigedoc_user_data', JSON.stringify(updatedUser));
    }
  }
  ...
  canUpdateCurrentRoute(currentUrl: string): boolean {
    ...
    for (const section of SIDEBAR_MENU) {
      for (const item of section.items) {
        if (item.subItems) {
          const found = item.subItems.find(sub => currentUrl.startsWith(sub.path));
          if (found) {
