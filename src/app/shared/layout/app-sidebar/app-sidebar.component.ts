...
import { SIDEBAR_MENU, MenuSection } from '../../../core/config/menu/sidebar.menu';
import { SesionService } from '@shared/services/sesion.service';
import { PermissionService } from '@core_db/security/permissions/permission.service';
  ...
  filterMenuByPermissions() {
    ...
    this.menuSections = SIDEBAR_MENU.map(section => {
      const filteredItems = section.items.map(item => {
        if (item.subItems) {
          const filteredSubItems = item.subItems.filter(sub => permittedModuleIds.has(sub.module_id));
          return {
            ...item,
